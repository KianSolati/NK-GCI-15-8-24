...
/* ************************************************** 
     ********* GETTING DATA FOR USER HISTORY ************
     ************************************************** */ 

// Get a single User with their business and logs
app.get('/UserWithLogs/:id', (req, res) => {
  ...
  const logsQuery = `
      SELECT Logs.logDate, Logs.logType, Logs.notes
      FROM Logs
      WHERE Logs.userID = ?;
  `;
...
/* ************************************************** 
     ********* GETTING DATA FOR BUSINESS HISTORY ************
     ************************************************** */ 

// Get a single Business with its logs
app.get('/BusinessWithLogs/:id', (req, res) => {
  const businessID = req.params.id;
  
  // Query to get business information
  const businessQuery = `
      SELECT businessName, contactType, contactDate, notes
      FROM Businesses
      WHERE businessID = ?;
  `;
  
  // Query to get logs associated with the business
  const logsQuery = `
      SELECT logDate, logType, notes
      FROM Logs
      WHERE businessID = ?;
  `;
  ...
  db.get(businessQuery, [businessID], (err, business) => {
      if (err) {
          ...
          return res.status(500).json({ error: 'Error fetching business information' });
      }
      ...
      // Execute the logs query if business is found
      db.all(logsQuery, [businessID], (err, logs) => {
          if (err) {
              console.error('Error fetching business logs:', err);
              return res.status(500).json({ error: 'Error fetching business logs' });
          }

          console.log(`Logs found: ${JSON.stringify(logs)}`);
          // Send the business data and logs back as JSON
          res.json({ business, logs });
      });
  });
});
